...
class PhysiologicalModel():
    ...
    def CobelliWrap(self, x, u, meal, p, model_data, malady):
        ...
        self.model_data = model_data
        self.malady = malady
        if self.malady == "type1":
            xdot = self.CobelliT1(0, self.x, self.u, self.meal, self.p,
                                self.model_data)
        elif self.malady == "exercise":
            xdot = self.CobelliEx(0, self.x, self.u, self.meal, self.p,
                                self.model_data)
        else:
    ...
    def Cobelli(self, t, x, u, meal, p, model_data):
        ...
        if self.malady=="healthy" or self.malady=="type1":
            h = 90
        elif self.malady=="type2":
            h = 130
    ...
    def CobelliEx(self, t, x, u, meal, p, model_data):
        import exercise
        [basal_heart_rate,heart_rate, W_ex] = exercise.Exercise(intensity="moderate").predict_heart_rate(basal_heart_rate=70,exercise_time=30)
        ...
        V_m = V_m0 + V_mx*X
        K_m = K_m0
        f_Y = ((Y_ex/(a_ex*heart_rate[t])**n)/(1 + (Y_ex/(a_ex*heart_rate[t]))**n)) #f(Y)
        U_id = ((V_m0*(1 + Beta*Y_ex) + V_mx*(1 + Alpha*Z_ex)*(X + I_b) - V_mx*I_b)*G_t)/(K_m*(1 - Gamma*Z_ex*W_ex[t]*(X + I_b))+ G_t )
        xdot[12] = -(Y_ex - (heart_rate[t] - basal_heart_rate))/T_hr #dYex_t
        xdot[13] = -(f_Y/T_in + 1/T_ex)*Z_ex + f_Y #dZ_t
    ...
    def RunSimulation(self, t, x, u, p, modelType, modelData, malady):
        ...
        xInitial0 = solver(self.CobelliWrap,x,
                           args=(u,0,p,modelData,self.malady))
...
def generate_data(mealVector, weight, runModel, filename, dt, end, malady=None):
    ...
    for i in xrange(ROWS):
        time = result[i]
        bolus = [x / 1801.8 for x in mealVector]
        #bolus = mealVector/1801.8
        G_p = runModel[0]
        #G_t = runModel[1][i]
        #I_l = runModel[2][i]
        #I_p = runModel[3][i]
        ...
        writer.writerow([weight, bolus, G_p]) # I_p, Ra])
                         #G_t, I_l, I_p,
...
def run_model(patient_type, modelData):
    ...
    # SET HOW MANY PATIENTS YOU WANT TO SIMULATE
    numPatients = 1

    dt = datetime.datetime.today()

    #fileHandle = open('data/data.csv',"a")
    #writer = csv.writer(fileHandle)

    for j in xrange(numPatients):
        weight = randint(70,90) #np.linspace(60,80,numDays)
        age = randint(18,65)
        bolus = []
        Gp = []
        flattenedGp = []
        ...
        for i in xrange(numDays):
            ...
            bob = patient.Patient(name="bob",age = age, body_weight = weight, malady ="type2")
            bob_malady = bob.get_malady()
            #print "Creating a patient named %s who is %s and weighs %s kgs" %(bob.get_name(),bob.get_malady(),bob.get_body_weight())

            #Create a meal
            meals = []

            breakfast = meal.Meal(hour=randint(4,11), minute=randint(0,59), carb_amount= randint(20,100))
            ...
            #Create a meal
            lunch = meal.Meal(hour=randint(12,17), minute=randint(0,59), carb_amount= randint(80,200))
            ...
            #Create a meal
            dinner = meal.Meal(hour=randint(18,22), minute=randint(0,59), carb_amount= randint(50,200))
            ...
            parameters = bob.set_parameters(bob.get_malady()).params()
            noise = 1 + np.random.normal(0,0.05,len(parameters))
            p = parameters
            #map(lambda x,y:x*y,parameters,noise)
            ...
            newModel = PhysiologicalModel(t, x, u, meals, p,
                                          patient_type, modelData)

            modelType = newModel.set_model_type(patient_type)

            runModel = newModel.RunSimulation( t, x, u, p, modelType, modelData, patient_type)['outputArray']

            #mealVector = newModel.RunSimulation( t, x, u, p, modelType, model_data, bob_malady)['mealVector']

            Gp.append(runModel[0])
            #bolus.append(mealVector)

        #Write data to file
        #generate_data(mealVector, weight, runModel,'data/data_%s_%i.csv' % (malady,i),dt, end, malady = bob.get_malady())
        #plot_everything.PlotEverything(runModel)
        flattenedGp = [val for sublist in Gp for val in sublist]
        flattenedGp = flattenedGp[:numDays*numTimeSteps]
        return flattenedGp
        # flattenedbolus = [val for sublist in bolus for val in sublist]
        # flattenedbolus = flattenedbolus[:numDays*numTimeSteps]
        #
        # data = [[weight], flattenedbolus, flattenedGp ,p]
        # flattenedData = [val for sublist in data for val in sublist]
        # #addToFile('data/data.csv', flattenedData)
        # fileHandle = open('data/data.csv',"a")
        # writer = csv.writer(fileHandle)
        # writer.writerow(flattenedData)
        # if len(flattenedData) != (2*numTimeSteps*numDays+36):
        #     print len(mealVector)
        #     print len(flattenedGp)
        #     print len(flattenedbolus)
        #     print j
        #     quit()
        # #generate_data(flattenedbolus, weight, flattenedGp,'data/data_%s_%i.csv' % (malady,j),dt, end, malady = bob.get_malady())


if __name__ == "__main__":
    import time
    start_time = time.time()
    run_model(patient_type,modelData)
